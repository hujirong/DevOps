...
def jarFile = props['jarFile']

def cntFile = workDir.listFiles().findAll { it.name ==jarFile }.size()

if (cntFile==1) {
   changeJarFile = jarFile
}
else
{
...
//delete extra files
workDir.eachFile { File f -> if (f.name  != jarFile ) f.delete() }
